...
export class SignController {
    async handleCreateSign(req, res) {
        try {
    ...
    async handleGetQuestions(req, res) {
        try {
    ...
    async handleUpdateSign(req, res) {
        try {
    ...
    async handleDeleteSign(req, res) {
        try {
            const id = Number(req.params.id);
            if (isNaN(id))
                return res.status(400).json({ error: "ID inválido." });
    ...
    async handleListAllSigns(req, res) {
        try {
